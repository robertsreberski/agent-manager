/*
  assistant-ui's ModelSelector, vendored and restyled to the cockpit's tokens.

  Composed through `ModelSelectorRoot`/`Trigger`/`Content` rather than the
  default `ModelSelector` export. That export renders `ModelSelectorModelContext`,
  which registers the selection into assistant-ui's `ModelContext` so it rides
  on `config.modelName` in every chat request. This cockpit does not use that
...
export const modelSelectorTriggerVariants = cva(
  "focus-visible:ring-[var(--ring)]/50 flex w-fit items-center justify-between gap-2 overflow-hidden rounded-md text-meta-sm whitespace-nowrap transition-colors outline-none focus-visible:ring-2 disabled:cursor-not-allowed disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-3.5",
  {
    variants: {
      variant: {
        outline:
          "border-[var(--border)] hover:bg-[var(--surface-selected)] hover:text-[var(--text)] border bg-transparent",
        ghost: "hover:bg-[var(--surface-selected)] hover:text-[var(--text)]",
        muted: "bg-secondary text-secondary-foreground hover:bg-secondary/80",
      },
      size: {
        default: "h-9 px-3 py-2",
        sm: "h-8 px-2.5 py-1.5 text-code-xs",
        lg: "h-10 px-4 py-2.5",
      },
    },
    defaultVariants: {
...
function ModelSelectorValue({
  ...
  return (
    ...
    >
      ...
      {effortName && (
        <span className="text-[var(--text-muted)] min-w-7.5 truncate text-center">
          {effortName}
        </span>
      )}
    </span>
...
function ModelSelectorContent({
  ...
  return (
    <PopoverContent
      ...
      className={cn(
        "bg-[var(--menu)]/95 w-72 min-w-(--radix-popover-trigger-width) overflow-hidden rounded-xl p-0 shadow-lg backdrop-blur-sm",
        className,
      )}
      {...props}
    >
      ...
      {children ?? (
        <>
          ...
          <span className="flex min-w-0 flex-col">
            ...
            {model.description && (
              <span className="text-[var(--text-muted)] truncate text-code-xs">
                {model.description}
              </span>
            )}
          </span>
...
function ModelSelectorEffort({
  ...
  return (
    <div
      ...
      {...props}
    >
      <span className="text-[var(--text-muted)] text-code-xs">{label}</span>
