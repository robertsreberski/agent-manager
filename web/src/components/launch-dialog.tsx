...
import { AlertTriangle, Bot, Check, Folder, ShieldAlert } from "lucide-react";
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl">
        <form onSubmit={submit} className="grid gap-5">
          <DialogHeader>
            <DialogTitle>Launch a managed session</DialogTitle>
            <DialogDescription>
              Managed sessions support semantic queueing, steering, interruption, and native attachment when the provider allows it.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="grid grid-cols-2 gap-2">
              {(["codex", "claude"] as const).map((item) => (
                <button
                  ...
                  className={cn(
                    "flex items-center gap-3 rounded-lg border p-3 text-left outline-none transition-colors hover:bg-accent focus-visible:ring-2 focus-visible:ring-ring",
                    provider === item && "border-primary bg-primary/5",
                  )}
                >
                  <Bot className={cn("size-5", item === "codex" ? "text-emerald-600" : "text-orange-600")} />
          ...
          <fieldset className="grid gap-2">
            <legend className="mb-1 text-sm font-medium">Access preset</legend>
            <div className="grid grid-cols-2 gap-2">
